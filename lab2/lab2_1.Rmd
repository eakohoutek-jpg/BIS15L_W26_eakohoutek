...
command+option+i
```{r}
getwd() #identify cuent woking diectory
```
...
```{r}
z=10 #this will work, but will ceate problems
```
...
```{r}
mean(c(venom_GT, chiron, veyron)) #to us 
```
...
```{r}
my_integer <- as.integer(my_numeric) #important when we wok with plots
```
